          ...
          <div className="form-group">
            ...
            <div className="grid grid-cols-2 gap-3 mt-1.5">
              <button
                type="button"
                onClick={() => setRole('看護師')}
                className={`w-full min-h-[76px] p-4 rounded-2xl border-4 text-left transition-all duration-200 active:scale-95 flex flex-col justify-between select-none ${
                  role === '看護師'
                    ? 'bg-sky-600 border-sky-600 text-white shadow-2xl ring-4 ring-sky-300 scale-[1.03] z-10'
                    : 'bg-slate-100 border-slate-300 text-slate-500 hover:bg-slate-200 opacity-60 hover:opacity-90'
                }`}
              >
                <div className="flex items-center justify-between w-full">
                  <span className={`font-extrabold text-base flex items-center gap-1.5 ${role === '看護師' ? 'text-white' : 'text-slate-700'}`}>
                    <Stethoscope className={`w-5 h-5 ${role === '看護師' ? 'text-white' : 'text-slate-500'}`} />
                    看護師
                  </span>
                  {role === '看護師' && (
                ...
                <p className={`text-[11px] font-medium mt-1 leading-tight ${role === '看護師' ? 'text-sky-100' : 'text-slate-400'}`}>
                  処置・バイタル・カルテ等
                </p>
              </button>
              ...
              <button
                ...
                className={`w-full min-h-[76px] p-4 rounded-2xl border-4 text-left transition-all duration-200 active:scale-95 flex flex-col justify-between select-none ${
                  role === '看護補助者'
                    ? 'bg-emerald-600 border-emerald-600 text-white shadow-2xl ring-4 ring-emerald-300 scale-[1.03] z-10'
                    : 'bg-slate-100 border-slate-300 text-slate-500 hover:bg-slate-200 opacity-60 hover:opacity-90'
                }`}
              >
                <div className="flex items-center justify-between w-full">
                  <span className={`font-extrabold text-base flex items-center gap-1.5 ${role === '看護補助者' ? 'text-white' : 'text-slate-700'}`}>
                    <HeartHandshake className={`w-5 h-5 ${role === '看護補助者' ? 'text-white' : 'text-slate-500'}`} />
                    看護補助者
                  </span>
                  {role === '看護補助者' && (
                ...
                <p className={`text-[11px] font-medium mt-1 leading-tight ${role === '看護補助者' ? 'text-emerald-100' : 'text-slate-400'}`}>
                  環境整備・ケア補助等
                </p>
              </button>
            </div>
